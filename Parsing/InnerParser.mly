%{
    ...
    let rec listOfNames_form_exp = function
        | { edesc = Name(id); etype=None } ->	[id]
        | { edesc = Attr(o,id); etype=None } -> (listOfNames_form_exp o)@[id]

    let rec listOfTypes_form_exp = function
        | { edesc = Name(id); etype=None } ->	Ref (Type.mk_type [] id)
      | { edesc = Attr(o,id); etype=None } -> Ref (Type.mk_type (listOfNames_form_exp o) id)
      | { edesc = Array(e,el); etype=None } -> Array(listOfTypes_form_exp e,List.length el)
...
expression:
  ...
  | o=expression LPAREN params=separated_list(COMMA,expression) RPAREN {
      match o with
      | { edesc = Name(id); etype=None } ->	{ edesc = Call(None,id,params); etype=None }
      | { edesc = Attr(o,id); etype=None } -> { edesc = Call(Some o,id,params); etype=None } }
  ...
  | o=expression DOT NEW id=qualifiedName LPAREN params=separated_list(COMMA,expression) RPAREN option(body(classContent* )) {
      match o with
      | { edesc = Name(n); etype=None } -> { edesc = New(Some n,id,params); etype=None } }
